...
const MAX_EVENTS = 2000; // Limits the database to the last ~2000 security events to use the ~250GB HDD space
...
export const db = {
  ...
  addEvent: async (event: Omit<SecurityEvent, 'id' | 'isRead' | 'timestamp'> & { timestamp?: Date }) => {
    try {
...
const deleteFile = (relativePath: string | undefined) => {
  ...
  try {
    // Ensure relativePath doesn't double slash if it already has a leading one
