...
class CameraManager {
  ...
  // 3. Test Mode: Manage internal mock engine
        ...
        if (!exists) {
          ...
          try {
            await db.addEvent({
              ...
              timestamp: timestamp, // Pass the correct timestamp from filename
            });
          } catch (dbErr) {
  ...
  private processEvent(config: Camera, msg: any) {
    try {
      ...
      // Check for Event Detection (Prioritized)
      // We look for common ONVIF event detection topics used by Tapo, Hikvision, and others.
      const isPerson = topic.includes('PeopleDetector/People') ||
        topic.includes('Device/EventDetection') ||
  ...
  public startTestMode() {
    ...
    const runTestStep = async () => {
      ...
      // 2. Look for existing recordings to generate a test event
      try {
        const recordingsDir = path.join(process.cwd(), 'public', 'recordings');
        ...
        log.info({ cameraId: config.id, cameraName: config.name }, 'TestEngine: generating mock event');
