...
### 🔐 0. Network Connectivity (CRITICAL)

Ensure that the Ubuntu server hosting Docker has network access to the cameras. If they are in the same local network, this is handled automatically. The application will fail to discover cameras if they are not reachable.
